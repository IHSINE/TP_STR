...
CONST(ResourceType, AUTOMATIC) distance = distance_id;

/*=============================================================================
 * Declaration of processes IDs
 */

/* Task move identifier */
#define move_id 0
CONST(TaskType, AUTOMATIC) move = move_id;

/* Task sonar identifier */
#define sonar_id 1
CONST(TaskType, AUTOMATIC) sonar = sonar_id;

/* Task touch identifier */
#define touch_id 2
CONST(TaskType, AUTOMATIC) touch = touch_id;
...
/* Alarm aT0 identifier */
#define aT0_id 0
CONST(AlarmType, AUTOMATIC) aT0 = aT0_id;

/* Alarm aT1 identifier */
#define aT1_id 1
CONST(AlarmType, AUTOMATIC) aT1 = aT1_id;

/* Alarm aT2 identifier */
#define aT2_id 2
CONST(AlarmType, AUTOMATIC) aT2 = aT2_id;
...
#include "tpl_memmap.h"
/*-----------------------------------------------------------------------------
 * Resource descriptor of resource distance
 *
 */
VAR(tpl_resource, OS_VAR) distance_rez_desc = {
  /* ceiling priority of the resource */  0,
 ...
 */
/*-----------------------------------------------------------------------------
 * Task move descriptor
 */
#define APP_Task_move_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task move function prototype
 */
FUNC(void, OS_APPL_CODE) move_function(void);
#define APP_Task_move_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 * Task move stack
 */
#define APP_Task_move_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) move_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_move_STOP_SEC_STACK
#include "tpl_memmap.h"

#define move_STACK {move_stack_zone, 500}
...
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) move_int_context;

#define move_CONTEXT &move_int_context
...
/*
 * Static descriptor of task move
 */
CONST(tpl_proc_static, OS_CONST) move_task_stat_desc = {
  /* context                  */  move_CONTEXT,
  /* stack                    */  move_STACK,
  /* entry point (function)   */  move_function,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task move
 */
VAR(tpl_proc, OS_VAR) move_task_desc = {
  /* resources                      */  NULL,
...
/*-----------------------------------------------------------------------------
 * Task sonar descriptor
 */
#define APP_Task_sonar_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task sonar function prototype
 */
FUNC(void, OS_APPL_CODE) sonar_function(void);
#define APP_Task_sonar_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 * Task sonar stack
 */
#define APP_Task_sonar_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) sonar_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_sonar_STOP_SEC_STACK
#include "tpl_memmap.h"

#define sonar_STACK {sonar_stack_zone, 500}
...
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) sonar_int_context;

#define sonar_CONTEXT &sonar_int_context
...
/*
 * Static descriptor of task sonar
 */
CONST(tpl_proc_static, OS_CONST) sonar_task_stat_desc = {
  /* context                  */  sonar_CONTEXT,
  /* stack                    */  sonar_STACK,
  /* entry point (function)   */  sonar_function,
  /* internal ressource       */  NULL,
  /* task id                  */  sonar_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task sonar
 */
VAR(tpl_proc, OS_VAR) sonar_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
/*-----------------------------------------------------------------------------
 * Task touch descriptor
 */
#define APP_Task_touch_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task touch function prototype
 */
FUNC(void, OS_APPL_CODE) touch_function(void);
#define APP_Task_touch_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 * Task touch stack
 */
#define APP_Task_touch_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) touch_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_touch_STOP_SEC_STACK
#include "tpl_memmap.h"

#define touch_STACK {touch_stack_zone, 500}
...
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) touch_int_context;

#define touch_CONTEXT &touch_int_context
...
/*
 * Static descriptor of task touch
 */
CONST(tpl_proc_static, OS_CONST) touch_task_stat_desc = {
  /* context                  */  touch_CONTEXT,
  /* stack                    */  touch_STACK,
  /* entry point (function)   */  touch_function,
  /* internal ressource       */  NULL,
  /* task id                  */  touch_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task touch
 */
VAR(tpl_proc, OS_VAR) touch_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
tpl_stat_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &move_task_stat_desc,
  &sonar_task_stat_desc,
  &touch_task_stat_desc,
 ...
 */
/*-----------------------------------------------------------------------------
 * Alarm aT0 descriptor
 *
 * This alarm does the activation of task touch
 */
...
CONST(tpl_task_activation_action, OS_CONST) aT0_action = {
  {
...
CONST(tpl_alarm_static, OS_CONST) aT0_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , aT0_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&aT0_action
};
...
VAR(tpl_time_obj, OS_VAR) aT0_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&aT0_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  400,
    /* date                         */  400,
...
/*-----------------------------------------------------------------------------
 * Alarm aT1 descriptor
 *
 * This alarm does the activation of task sonar
 */
...
CONST(tpl_task_activation_action, OS_CONST) aT1_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  sonar_id
...
CONST(tpl_alarm_static, OS_CONST) aT1_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , aT1_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&aT1_action
};
...
VAR(tpl_time_obj, OS_VAR) aT1_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&aT1_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  800,
    /* date                         */  800,
...
/*-----------------------------------------------------------------------------
 * Alarm aT2 descriptor
 *
 * This alarm does the activation of task move
 */
...
CONST(tpl_task_activation_action, OS_CONST) aT2_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  move_id
...
CONST(tpl_alarm_static, OS_CONST) aT2_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , aT2_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&aT2_action
};
...
VAR(tpl_time_obj, OS_VAR) aT2_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&aT2_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  600,
    /* date                         */  600,
